...
class ErraticCat(Cat):
    ...
    def __init__(self, grid_size: int, tile_size: int):
        ...
        self.personalities = {
            "batmeow": BatmeowCat(grid_size, tile_size),
            "mittens": MittensCat(grid_size, tile_size),
            "paotsin": PaotsinCat(grid_size, tile_size),
            "peekaboo": PeekabooCat(grid_size, tile_size),
            "squiddyboi": SquiddyboiCat(grid_size, tile_size)
        }
    ...
    def move(self) -> None:
        ...
        if self.personality_counter == 0 or self.personality_counter >= 5:
            self.current_personality = random.choice(list(self.personalities.values()))
            self.personality_counter = 0

        self.personality_counter += 1

        self.current_personality.pos = self.pos.copy()
        self.current_personality.player_pos = self.player_pos.copy()
        self.current_personality.prev_player_pos = self.prev_player_pos.copy()
        self.current_personality.last_player_action = self.last_player_action

        self.current_personality.move()

        self.pos = self.current_personality.pos.copy()
...
class WayneCat(Cat):
    ...
    def move(self) -> None:
        ...
        if player_row == 5 and player_col == 6:
            # Teleport directly onto the player
            self.pos[0] = player_row
            self.pos[1] = player_col
            self.visual_pos[0] = float(player_row)  # instantly update visual
            self.visual_pos[1] = float(player_col)
        else:
            # Teleport randomly at least 2 tiles away from the player
            possible_positions = [
                (r, c)
                for r in range(self.grid_size)
                for c in range(self.grid_size)
                if abs(r - player_row) + abs(c - player_col) >= 2
            ]

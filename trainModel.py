...
def extract_numeric_features(word: str):
    ...
    suffixes_eng = ['ing', 'ed', 'tion', 's', 'ly', 'able', 'ment']
...
def main():
    ...
    # splitting dataset
    min_count = df['label_norm'].value_counts().min()
    stratify_option = y if min_count >= 2 else None
    if stratify_option is None:
        print("Warning: Some classes have <2 samples; skipping stratified split")

    X_train, X_temp, y_train, y_temp = train_test_split(
        X_combined, y, test_size=0.3, random_state=RANDOM_STATE, stratify=stratify_option
    )

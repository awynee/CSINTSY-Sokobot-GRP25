...
def tag_language(tokens: List[str]) -> List[str]:
    ...
    # loading the trained model and related tools from the saved file
    saved_data = joblib.load(model_file)
    model = saved_data["model"]
    le = saved_data["label_encoder"] # label encoder for converting numbers to tags
    vectorizer = saved_data["char_vectorizer"] # character n-gram vectorizer
    num_featureNames = saved_data["num_featureNames"]

    # extracting numeric features from the input tokens
    X_num = [extractNumericFeatures(w) for w in tokens]
    X_numSparse = sparse.csr_matrix([list(d.values()) for d in X_num])
    
    # extracting character-level features from the tokens
    X_char = vectorizer.transform(tokens)

    # combining numeric & character features into 1 matrix
    X_combined = sparse.hstack([X_numSparse, X_char])

    preds_encoded = model.predict(X_combined) # makes predictions using the trained model
    preds = le.inverse_transform(preds_encoded) # converts numeric predictions back to string

    tags = [str(p) for p in preds]

    # post-processing
    for i, token in enumerate(tokens):  #checks each word and fixes words that are likely names    
        if token[0].isupper() and i != 0 and tags[i] in ["FIL", "ENG"]:
            if token.isalpha() and len(token) > 1:  # label as OTH if it looks like a name
                tags[i] = "OTH"
